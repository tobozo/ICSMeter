#line 1 "/home/runner/work/ICSMeter/ICSMeter/include/assets/index_html.hpp"
#pragma once


namespace ICSMeter
{


  // cheap substitute for ArduinoJSON :-)
  const char subscribe_json_format[] = R"JSON(
{
  "tty":"%s",
  "baud_rate":"%s",
  "command":"%s",
  "polling_rate":"%s",
  "host":"%s"
}
  )JSON";



  #if defined ARDUINO_M5STACK_Core2

    #define CSS_BACKGROUND_IMAGE "background-image: url('data:image/png;base64,');"

    #define CSS_BUTTON_STYLE "position: relative; top: -12px; left: 0px; width: 45px; height: 31px;"


  #elif defined ARDUINO_M5Stack_Core_ESP32

    #define CSS_BACKGROUND_IMAGE "background-image: url('data:image/png;base64,');"

    #define CSS_BUTTON_STYLE "position: relative; top: 0px; left: 0px; width: 58px; height: 31px; background-image: url('data:image/png;base64,');"

  #else
    #error "Please select either M5Stack-Core-ESP32 or M5Stack-Core2 board"
  #endif


  const char index_html[] = R"HTML(
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>ICSMeter Screen Capture</title>
<style>
  html, body {
    font-family: Arial, sans-serif;
    margin: 0;
    padding: 0;
    width: 100%;
    height: 100%;
    display: table;
    text-align: center;
    background-color: #313131;
  }
  table {
    width: 396px;
    height: 396px;
    border-color: #000;
    margin-left: auto;
    margin-right: auto;
    )HTML" CSS_BACKGROUND_IMAGE R"HTML(
  }
  td { padding-top:  10px; }
  ul li { display: inline-block; margin-right: 40px; }
  #content { display: table-cell; text-align: center; vertical-align: middle; }
  .screen { position: relative; top: 40px; left: 0px; width: 320px; height: 240px; }
  .button { )HTML" CSS_BUTTON_STYLE R"HTML( }
</style>
</head>
<body>
<div id="content">
  <table>
    <tr><td>
      <a href="/">
        <img class="screen" alt="Screen" src="screenshot.bmp"/>
      </a>
    </td></tr>
    <tr><td>
      <ul>
        <li><a href="buttonLeft"><div type="button" class="button"></div></a></li>
        <li><a href="buttonCenter"><div type="button" class="button"></div></a></li>
        <li><a href="buttonRight"><div type="button" class="button"></div></a></li>
      </ul>
    </td></tr>
  </table>
</div>
<script>


const data =
{
  tty          : '/dev/ttyUSB1',
  baud_rate    : '115200',
  command      : 'fefea4e01502fd',
  polling_rate : '1',
  host         : 'icsmeter.local'
};


function unsubscribe()
{
  fetch('http://localhost:1234/unsubscribe', {
    method: 'POST', // or 'PUT'
    mode:'no-cors', // no-cors, *cors, same-origin
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify(data),
  })
  .then(response => response.json())
  .then(data => {
    console.log('Success:', data);
  })
  .catch((error) => {
    console.error('Error:', error);
  });
}


function subscribe()
{
  fetch('http://localhost:1234/subscribe', {
    method: 'POST', // or 'PUT'
    mode:'no-cors', // no-cors, *cors, same-origin
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify(data),
  })
  .then(response => response.json())
  .then(data => {
    console.log('Success:', data);
  })
  .catch((error) => {
    console.error('Error:', error);
  });
}

</script>
<button onclick=subscribe() style="display:none">subscribe</button>
<button onclick=unsubscribe() style="display:none">unsubscribe</button>
</body>
</html>
)HTML";


};
